...
class GatewayMvxAMM(GatewayEVMAMM):
    """
    ...
    def network_transaction_fee(self, new_fee: TokenAmount):
        self._network_transaction_fee = TokenAmount('EGLD', Decimal(0))
    ...
    async def update_order_status(self, tracked_orders: List[GatewayInFlightOrder]):
        ...
        for tracked_order, tx_details in zip(tracked_orders, update_results):
            ...
            if tx_receipt is not None:
                if tx_status == 1:
                    gas_used: int = tx_receipt["gasLimit"]
                    gas_price: Decimal = tx_receipt["gasPrice"]
                    fee: Decimal = Decimal(str(gas_used)) * Decimal(str(gas_price)) / Decimal(str(1e18))
    ...
    async def cancel_outdated_orders(self, cancel_age: int) -> List[CancellationResult]:
        """
        We do not cancel transactions on Multiversx network.
        """
        return []
